...
- 네이비+민트 (모던) — primary `#02567D`, secondary `#0c9fa5`, accent `#FF9084`
...
(선교 카드(mission-card)는 admin > 선교에서 내용을 채운 뒤 노출하는 고급 섹션이라 인터뷰에서는 제외합니다.)

저장:
```json
{ "sections": ["greeting", "vision", "worship", "featured-sermon", "sermons-quad", "quick-actions", "gallery-strip", "online-giving", "location"] }
```
