...
export const metadata = { title: "특별영상" };
...
export default async function VideosPage({ searchParams }: Props) {
  ...
  const { data, total } = await getVideos({ page, perPage: PER_PAGE });

  return (
    <>
      <PageHero eyebrow="VIDEOS" title="특별영상" />

      <div className="container mx-auto px-4 py-12 md:py-16">
        {data.length === 0 ? (
          <EmptyState message="등록된 특별영상이 없습니다." />
        ) : (
          <>
                  ...
                  >
                    ...
                    <div className="p-5">
                      ...
                      <div className="mt-3 flex items-center justify-between text-sm text-warm-gray">
                        <span>{v.performer ?? CHURCH.name}</span>
                        <time dateTime={v.video_date}>
                          {format(new Date(v.video_date), "yyyy.MM.dd")}
                        </time>
                      </div>
                    </div>
                  </Link>
